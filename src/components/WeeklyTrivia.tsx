...
export const WeeklyTrivia: React.FC<WeeklyTriviaProps> = ({ currentUser }) => {
  ...
  const canDeleteQuiz = (quiz: WeeklyQuiz) => {
    if (!isAdmin) return false;
    if (['Super Admin', 'Head', 'Vice'].includes(currentUser.role)) return true;
    if (quiz.createdBy === currentUser.id) return true;
    if (!quiz.createdBy) return true;
    return false;
  };
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 animate-fade-in max-w-7xl mx-auto" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      {/* Quizzes List / Switcher if more than 1 quiz */}
      {quizzes.length > 1 && (
        <div className="bg-white dark:bg-slate-900 p-4 sm:p-5 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-xs font-black uppercase tracking-wider text-slate-700 dark:text-slate-300 flex items-center gap-2">
              ...
              <span>{isAr ? 'المسابقات والتحديات المتاحة' : 'Available Contests'} ({quizzes.length})</span>
            </h3>
            <span className="text-[11px] text-slate-400 font-bold">
              {isAr ? 'اضغط على أي مسابقة لعرضها وحلها أو استعراض من أنشأها' : 'Click any quiz to select and view creator'}
            </span>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {quizzes.map(q => {
              const isSelected = q.id === (activeQuiz?.id || selectedQuizId);
              const sub = db.getQuizSubmissions(q.id).find(s => s.userId === currentUser.id);
              const creatorName = q.createdByName || (isAr ? 'إدارة المنصة' : 'Platform Admin');

              return (
                <button
                  key={q.id}
                  onClick={() => handleSelectQuiz(q.id)}
                  className={`p-3.5 rounded-2xl text-start transition-all border flex flex-col justify-between gap-2.5 relative cursor-pointer ${
                    isSelected
                      ? 'bg-purple-50/90 dark:bg-purple-950/40 border-purple-500 ring-2 ring-purple-400/40 shadow-sm'
                      : 'bg-slate-50/70 dark:bg-slate-800/40 border-slate-200/80 dark:border-slate-700/80 hover:border-purple-300'
                  }`}
                >
                  <div className="space-y-1 w-full">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-[10px] font-black px-2 py-0.5 rounded-full bg-purple-100 dark:bg-purple-950 text-purple-700 dark:text-purple-300">
                        {(!q.committee || q.committee === 'All') ? (isAr ? 'عامة' : 'General') : q.committee}
                      </span>
                      {sub ? (
                        <span className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 flex items-center gap-1 bg-emerald-100/80 dark:bg-emerald-950/60 px-2 py-0.5 rounded-full">
                          <CheckCircle2 className="w-3 h-3" />
                          <span>{isAr ? 'تم الحل' : 'Done'}</span>
                        </span>
                      ) : (
                        <span className="text-[10px] font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/50 px-2 py-0.5 rounded-full">
                          ⚡ {q.pointsReward || 50} pts
                        </span>
                      )}
                    </div>
                    <h4 className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white line-clamp-1">
                      {q.title || (isAr ? 'مسابقة أسبوعية' : 'Weekly Quiz')}
                    </h4>
                  </div>

                  {/* Creator Info on the Quiz Card */}
                  <div className="pt-2 border-t border-slate-200/60 dark:border-slate-700/60 w-full flex items-center justify-between gap-2 text-[11px]">
                    <div className="flex items-center gap-1.5 min-w-0">
                      <div className="w-5 h-5 rounded-full bg-gradient-to-tr from-purple-500 to-indigo-600 text-white font-black text-[9px] flex items-center justify-center shrink-0 overflow-hidden">
                        {q.createdByAvatar ? (
                          <img src={q.createdByAvatar} alt="" className="w-full h-full object-cover" />
                        ) : (
                          creatorName.charAt(0)
                        )}
                      </div>
                      <span className="font-semibold text-slate-700 dark:text-slate-300 truncate">
                        {isAr ? `أنشأها: ${creatorName}` : `By: ${creatorName}`}
                      </span>
                    </div>
                    {q.createdByRole && (
                      <span className="text-[9px] font-bold text-purple-600 dark:text-purple-400 bg-purple-100/60 dark:bg-purple-950/60 px-1.5 py-0.5 rounded shrink-0">
                        {q.createdByRole}
                      </span>
                    )}
                  </div>
              ...
              <div className="flex flex-wrap items-start justify-between gap-4 pb-5 border-b border-slate-100 dark:border-slate-800">
                <div className="space-y-2.5 max-w-xl">
                  ...
                  <div className="flex items-center gap-2 pt-1 flex-wrap">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-purple-50/80 dark:bg-slate-800/90 border border-purple-200/70 dark:border-slate-700 rounded-2xl text-xs">
                      <div className="w-6 h-6 rounded-full bg-gradient-to-tr from-purple-500 to-indigo-600 text-white font-black text-[10px] flex items-center justify-center shrink-0 shadow-sm overflow-hidden">
                        {activeQuiz.createdByAvatar ? (
                          <img src={activeQuiz.createdByAvatar} alt="" className="w-full h-full object-cover" />
                        ) : (
                          (activeQuiz.createdByName || 'إ').charAt(0)
                        )}
                      </div>
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <span className="text-slate-500 dark:text-slate-400 font-medium text-[11px]">
                          {isAr ? 'أنشأ المسابقة القائد:' : 'Created by Leader:'}
                        </span>
                        <span className="font-black text-purple-800 dark:text-purple-300">
                          {activeQuiz.createdByName || (isAr ? 'إدارة المنصة' : 'Platform Admin')}
                        </span>
                        {activeQuiz.createdByRole && (
                          <span className="text-[10px] font-bold px-2 py-0.5 bg-purple-100 dark:bg-purple-950 text-purple-700 dark:text-purple-300 rounded-full border border-purple-200 dark:border-purple-800">
                            {activeQuiz.createdByRole}
                          </span>
                        )}
                        {activeQuiz.createdBy === currentUser.id && (
                          <span className="text-[10px] font-black px-2 py-0.5 bg-amber-100 dark:bg-amber-950 text-amber-800 dark:text-amber-200 rounded-full border border-amber-200 dark:border-amber-800">
                            {isAr ? 'مسابقتك ✍️' : 'Your Quiz ✍️'}
                          </span>
                        )}
                      </div>
                    </div>

                    {activeQuiz.createdAt && (
                      <span className="text-[11px] text-slate-400 flex items-center gap-1 font-medium">
                        <Clock className="w-3.5 h-3.5" />
                        <span>{new Date(activeQuiz.createdAt).toLocaleDateString(isAr ? 'ar-EG' : 'en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {canDeleteQuiz(activeQuiz) && (
                    <button
                      onClick={() => handleDeleteQuiz(activeQuiz.id)}
                      className="px-3 py-1.5 rounded-2xl bg-red-50 dark:bg-red-950/40 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/40 transition-all flex items-center gap-1.5 text-xs font-bold border border-red-200/60 dark:border-red-800/40 cursor-pointer"
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="w-full max-w-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6 my-6 modal-panel-animate max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center gap-2.5 p-3 rounded-2xl bg-purple-50/80 dark:bg-purple-950/40 border border-purple-200/60 dark:border-purple-800/60 text-xs text-purple-900 dark:text-purple-200">
              <div className="w-7 h-7 rounded-xl bg-purple-600 text-white font-black text-xs flex items-center justify-center shrink-0">
                {currentUser.avatar ? (
                  <img src={currentUser.avatar} alt="" className="w-full h-full rounded-xl object-cover" />
                ) : (
              ...
              <div className="min-w-0 flex-1">
                <span className="font-bold text-slate-500 dark:text-slate-400 text-[11px] block">
                  {isAr ? 'منشئ المسابقة (سيظهر للجميع):' : 'Quiz Creator (Visible to all):'}
                </span>
                <span className="font-black text-purple-700 dark:text-purple-300">
                  {currentUser.fullName}
                </span>
                <span className="text-[10px] font-bold mx-1.5 px-2 py-0.5 rounded-full bg-purple-200/70 dark:bg-purple-900/60 text-purple-800 dark:text-purple-200">
                  {currentUser.role}
                </span>
              </div>
            </div>
