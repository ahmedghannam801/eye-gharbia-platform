...
import {
  ...
  RefreshCw, Building2, FileUp, MessageSquare, Star, Sparkles, Eye, Shield
} from 'lucide-react';
                    ...
                    {!iCheckedIn && currentUser.role === 'Member' && mtg.status === 'Open' && (
                      <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/30 rounded-2xl p-5 border border-blue-200/60 dark:border-blue-800/60 shadow-sm space-y-4">
                        ...
                        <div className="space-y-3">
                          ...
                          {/* Feedback & Suggestions Textarea (Leader-Only Visibility) */}
                          <div className="space-y-1">
                    ...
                    <div>
                      <div className="flex items-center justify-between gap-2 mb-2 flex-wrap">
                        ...
                        {isLeaderOrAdmin(currentUser) && att.some(a => a.feedback || a.rating) && (
                          <button
                            ...
                            className="flex items-center gap-1.5 px-3 py-1 bg-amber-50 dark:bg-amber-950/40 text-amber-800 dark:text-amber-300 hover:bg-amber-100 border border-amber-300 dark:border-amber-700/60 rounded-xl text-xs font-black transition-all shadow-sm cursor-pointer"
                          >
                            <MessageSquare className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400" />
                            <span>{isAr ? `💬 آراء ومقترحات الأعضاء (${att.filter(a => a.feedback || a.rating).length})` : `View Feedback (${att.filter(a => a.feedback || a.rating).length})`}</span>
                          </button>
                        )}
                      </div>
                      ...
                      {att.length === 0 ? (
                        ...
                        <div className="space-y-2 max-h-60 overflow-y-auto">
                          {att.map(a => (
                            <div key={a.id} className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-700 space-y-1.5">
                              <div className="flex items-center gap-3">
                                <UserCheck className="w-3.5 h-3.5 text-emerald-500 shrink-0" />
                                <div className="flex-1 min-w-0">
                                  <p 
                                    className="text-[11px] font-black text-slate-800 dark:text-slate-100 cursor-pointer hover:text-eye-brand hover:underline flex items-center gap-2"
                                    onClick={() => onNavigateToView?.('profile', a.memberId)}
                                  >
                                    <span>{a.memberName}</span>
                                    {isLeaderOrAdmin(currentUser) && a.rating && (
                              ...
                              {/* Feedback Box (Visible only to Leaders/Admins) */}
                              {isLeaderOrAdmin(currentUser) && a.feedback && (
                                <div className="ms-6 p-2.5 bg-amber-50/70 dark:bg-amber-950/30 border border-amber-200/70 dark:border-amber-800/50 rounded-xl text-xs text-amber-900 dark:text-amber-200">
                                  <div className="flex items-center gap-1 text-[10px] font-black text-amber-800 dark:text-amber-300 mb-0.5">
                                    <MessageSquare className="w-3 h-3 text-amber-600" />
                                    <span>{isAr ? 'ملاحظة ومقترح العضو:' : 'Member Feedback:'}</span>
                                  </div>
                                  <p className="text-[11px] font-medium leading-relaxed whitespace-pre-line">{a.feedback}</p>
                                </div>
                              )}
                            </div>
                          ))}
